...
def search_repository_text(
    ...
    target_argument = get_repository_shell_target_argument(repo_root, target_path)
    ...
    shell_command = "command -v rg >/dev/null 2>&1 && " + quote_shell_args(command)
    return_code, output = shell_executor.run(shell_command)
    if return_code not in {0, 1}:
        raise ValueError(output.strip() or "ripgrep failed")
