...
class ProjectHandlePersistence:
    ...
    def get_project_handle_fields(self, project_id: int) -> dict | None:
        session = self.db_connection or SessionLocal()
        try:
            stmt = select(Project).where(Project.project_id == project_id)
            result = session.execute(stmt).scalar_one_or_none()
...
class ProjectBindingPersistence:
    def __init__(self, db_connection=None):
        self.db_connection = db_connection

    def get_project_binding_fields(self, project_id: int) -> dict | None:
        session = self.db_connection or SessionLocal()
        try:
            stmt = select(Project).where(Project.project_id == project_id)
            result = session.execute(stmt).scalar_one_or_none()
